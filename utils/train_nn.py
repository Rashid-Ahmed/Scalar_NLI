...
from utils.dataset_transformer import NLIDataset
import os
import numpy as np
...
def train_model(MODEL_TYPE , DATA_LEN , CHECKPOINT_DIR, device, FILE_NAME ,EPOCHS, STEP_SIZE, BATCH_SIZE, load_model = False):
    ...
    BEST_ACC = 0
    if load_model == True:
      model.load_state_dict(torch.load(os.path.join(CHECKPOINT_DIR, 'model_roberta.ckpt')))
    for epoch in range(EPOCHS): 
        ...
        dataset = NLIDataset(MODEL_TYPE, csv_file = FILE_NAME, chunk_size = BATCH_SIZE, len_dataset = DATA_LEN)
        ...
        for i in range(len(dataset)): 
          ...
          labels = torch.from_numpy(np.array(targets)).to(device)
          ...
          if iteration < train_batches:
            ...
            test_truth = torch.cat((test_truth, labels.cpu()), 0)
              ...
              if test_acc > BEST_ACC:
                  BEST_ACC = test_acc
                  torch.save(model.state_dict(), os.path.join(CHECKPOINT_DIR, 'model_roberta.ckpt'))
